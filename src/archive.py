...
class Archive:
    ...
    def populate_file_info(self, limit = 1000):
        ...
        try:
            ...
            rslt = self.db.populate_archive_file_info(files, fmap)
            print(f'ARCHIVE.POPULATE_FILE_INFO: {rslt} Populated')
            rslt = self.db.complete_file_info_population()
            print(f'ARCHIVE.POPULATE_FILE_INFO: {rslt} Completed')
            self._close()
            return cnt
    ...
    def move_files_to_archive_dir(self, archive_date):
        ...
        if not re.match(r'^20\d{2}\d{2}\d{2}$', archive_date):
            print('MOVE_FILES_TO_ARCHIVE_DIR: Invalid Date ' + archive_date)
            return 0
        try:
            cnt = 0
            self._open()
    ...
    def delete_folder(self, archive_date):
        """
        ...
        try:
            ...
            print('ARCHIVE.DELETE_FOLDER: Folder Deleted ' + archive_date)
            rslt = self.db.complete_file_delete()
